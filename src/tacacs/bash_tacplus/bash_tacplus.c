 ...
 */
int on_shell_execve(char *user, char *cmd, char **argv)
{
...
int
send_acct_msg(int tac_fd, char * user, char *cmd)
{   
    char buf[128];
    struct tac_attrib *attr;
    int retval;
    ...
    int type = TAC_PLUS_ACCT_FLAG_START;
    char *tty = "vty";
    char *host = "UNK";
 ...
 * Tacacs authorization.
 */
void command_account(char *user, char *cmd)
{
    int server_idx = 0;
    int server_fd = 0;
    int retval = 0;
    ...
    for(; server_idx < tac_srv_no; server_idx++) {
        server_fd = tac_connect_single(tac_srv[server_idx].addr,
                                       tac_srv[server_idx].key,
                                       tac_source_addr,
                                       tac_timeout,
                                       __vrfname);
        if(server_fd < 0) {
            syslog(LOG_WARNING, "connection to %s failed (%d) to send"
                " accounting record: %m",
                tac_ntop(tac_srv[server_idx].addr->ai_addr), server_fd);
            continue;
        }
        retval = send_acct_msg(server_fd, user, cmd);
        if(retval) {
            syslog(LOG_WARNING, "error sending accounting record to %s: %m",
                tac_ntop(tac_srv[server_idx].addr->ai_addr));
        }
